...
// Query: *[_type == "event"  && dateTime(date) >= dateTime($monthStart)  && dateTime(date) <= dateTime($monthEnd)] | order(date asc) {  _id, title, "slug": slug.current, date, endDate,  location, eventType, status}
