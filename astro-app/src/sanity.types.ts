...
// Query: *[_type == "team" && $email in members[]->email][0]{  _id,  name,  semester,  maxMembers,  githubRepoUrl,  discordChannelUrl,  project->{    _id,    title,    "slug": slug.current,    content,    sponsor->{      _id,      name,      "slug": slug.current,      logo{        asset->{ _id, url, metadata { lqip, dimensions } },        alt      }    },    technologyTags  },  "pm": pm->{ name, email, githubUsername },  "assistantPm": assistantPm->{ name, email, githubUsername },  "members": members[]->{ _id, name, email, githubUsername, discordUsername },  "memberCount": count(members),  "isPM": pm->email == $email,  "isAPM": assistantPm->email == $email,  teamResources[]{ label, url, category }}
