# Story 2.8: fulldev/ui Schemas — Media & Miscellaneous
...
- [ ] Task 2: Create Sanity schemas (AC: #1)
  - [ ] 2.1 Create schema files for logos-1, logos-2, logos-3 in `studio/src/schemaTypes/blocks/` using `defineBlock`
  - [ ] 2.2 Create schema files for images-1, images-2
  - [ ] 2.3 Create schema files for video-1, video-2, video-3
  - [ ] 2.4 Create schema files for videos-1 through videos-4
  - [ ] 2.5 Create schema files for contact-1, contact-2, contact-3
  - [ ] 2.6 Create schema files for stats-1, stats-2, stats-3
  - [ ] 2.7 Create schema file for skeletons-1
  - [ ] 2.8 Ensure each schema's fields match the component's props exactly (name, type, validation)
...
`skeletons-1` is a loading/placeholder component. Determine whether it needs CMS-managed content or if it should remain static. If purely decorative, the schema can be minimal (just the `_type` field from `defineBlock` base fields).

### Shared Files (Coordination with Stories 2.4–2.7)
...
Work on branch `feat/2.8-ui-blocks-media-misc`. Merge sequentially with other parallel stories to resolve additive conflicts.

### Dependencies

- **Requires:** Story 1.3 (schema infrastructure — `defineBlock` helper)
- **Parallel with:** Stories 2.4, 2.5, 2.6, 2.7
- **No dependency on:** Stories 2.1, 2.2, 2.3 (custom block schemas are independent)

### References

- [Source: astro-app/src/components/blocks/logos-*.astro — Component prop interfaces]
- [Source: astro-app/src/components/blocks/images-*.astro — Component prop interfaces]
- [Source: astro-app/src/components/blocks/video-*.astro — Component prop interfaces]
- [Source: astro-app/src/components/blocks/videos-*.astro — Component prop interfaces]
- [Source: astro-app/src/components/blocks/contact-*.astro — Component prop interfaces]
- [Source: astro-app/src/components/blocks/stats-*.astro — Component prop interfaces]
- [Source: astro-app/src/components/blocks/skeletons-*.astro — Component prop interfaces]
