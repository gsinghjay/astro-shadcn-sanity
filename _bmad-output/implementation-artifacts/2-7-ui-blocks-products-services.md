# Story 2.7: fulldev/ui Schemas — Products & Services
...
- [ ] Task 2: Create Sanity schemas (AC: #1)
  - [ ] 2.1 Create schema file for product-1 in `studio/src/schemaTypes/blocks/` using `defineBlock`
  - [ ] 2.2 Create schema files for products-1 through products-5
  - [ ] 2.3 Create schema files for services-1 through services-7
  - [ ] 2.4 Create schema files for pricings-1, pricings-2, pricings-3
  - [ ] 2.5 Create schema files for reviews-1 through reviews-5
  - [ ] 2.6 Ensure each schema's fields match the component's props exactly (name, type, validation)
...
Work on branch `feat/2.7-ui-blocks-products-services`. Merge sequentially with other parallel stories to resolve additive conflicts.

### Dependencies

- **Requires:** Story 1.3 (schema infrastructure — `defineBlock` helper)
- **Parallel with:** Stories 2.4, 2.5, 2.6, 2.8
- **No dependency on:** Stories 2.1, 2.2, 2.3 (custom block schemas are independent)

### References

- [Source: astro-app/src/components/blocks/product-*.astro — Component prop interfaces]
- [Source: astro-app/src/components/blocks/products-*.astro — Component prop interfaces]
- [Source: astro-app/src/components/blocks/services-*.astro — Component prop interfaces]
- [Source: astro-app/src/components/blocks/pricings-*.astro — Component prop interfaces]
- [Source: astro-app/src/components/blocks/reviews-*.astro — Component prop interfaces]
