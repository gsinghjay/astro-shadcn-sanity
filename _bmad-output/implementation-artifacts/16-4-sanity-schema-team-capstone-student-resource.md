...
- Fixed GROQ projection: story spec had `description` in project projection but project schema has no `description` field — removed from projection (project has `content` (portableText) and `outcome` (text); if Story 16.5 needs project body, add explicitly with portableText rendering awareness)
...
- Fixed GROQ: added `| order(_createdAt desc)` before `[0]` so returning students (multiple semesters) get their most recent team
- Added early-return guard in `getStudentTeam()` for empty email to avoid unnecessary API calls
- Tightened test helper types: replaced bare `any` with `SchemaFieldRecord` alias + typed `fieldNames` parameter
...
- `playwright-report/index.html` — build artifact (not application code)
