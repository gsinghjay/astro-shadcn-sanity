...
- **Follow-up 2026-05-01 — Story 24.1.5 (post-deploy hotfix to 24.1):**
  - 24.1's Path A (Studio forwards `client.config().token` as a Bearer to the Worker) was incompatible with capstone Studio's actual auth mode — cookie auth, where the session is stored in an `httpOnly` cookie scoped to `*.api.sanity.io` and is unreadable from JS. `client.config().token` is `undefined` in production, so the tool shipped a "Studio session has no auth token" error and never reached the Worker. H-1 stayed closed (no bearer in the bundle), but the tool was non-functional.
  - 24.1.5 replaces the bearer-forwarding model with a server-side **project-membership check**: the Studio cookie-fetches `https://<projectId>.api.sanity.io/v1/users/me` (with `credentials: 'include'`) to obtain the current user's id, forwards it as `X-Sanity-User-Id`, and the Worker independently calls `https://api.sanity.io/v2024-10-01/projects/<projectId>/users/<id>` with a Worker-only Sanity API token (`SANITY_PROJECT_READ_TOKEN`, `sk`-prefixed, Viewer permissions) to confirm membership. 200 with matching `id` → admit; anything else → 403.
  - All bearer plumbing dropped from both ends: `MAX_BEARER_LENGTH`, `hashToken()`, `getStudioUser()`, `INTROSPECTION_*`, `Authorization: Bearer` headers. Membership cache keyed directly on user id (not a secret) — SHA-256 wrapper dropped. Origin gate, fail-closed env check, 60s/100-entry LRU cache shape, and bundle-scrub all retained.
  - Bundle-scrub widened to also reject `sk[A-Za-z0-9_-]{60,}` literals (`sk`-prefixed Sanity tokens; body-length floor raised above natural identifier length to avoid false positives like `skipAnimationFrameInResizeObserver`) plus the env-var names `SANITY_PROJECT_READ_TOKEN`, `SANITY_API_READ_TOKEN`, `SANITY_API_WRITE_TOKEN` — none of which belong in any Studio chunk.
  - **Residual risk (explicitly accepted):** an attacker who (a) knows the Worker URL, (b) can forge the `Origin` header (curl/non-browser HTTP clients can — browsers can't), AND (c) knows or guesses any current project member's Sanity user ID, can read the acceptances list. Sanity user IDs are project-scoped, not advertised, and not in the bundle, but they are not cryptographic secrets. Membership is restricted to two roles (`administrator` + `viewer`), both intentionally trusted with sponsor PII; the endpoint returns name/email/role/timestamp/version metadata only (no financial/credential data); the capstone roster is small and vetted.
  - **Path B (Sanity Function mints signed JWT) remains the documented escalation** if the residual threat profile ever stops being acceptable (broader membership, more roles, more sensitive columns). Server-side verification would swap from "is this id a project member" to "is this signed claim fresh and valid".
